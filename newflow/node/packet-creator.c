...
packet_t* 
create_K173713(int beacon_count,int report_period,int beacon_period) //defining create function of our packet
{  
  packet_t* p = create_packet_empty();
  if (p != NULL){
    p->header.net = conf.my_net;
    ...
    p->header.src = conf.my_address;
    increment_address(&(p->header.src),d);
    p->header.typ = K173713;
    p->header.nxh = conf.nxh_vs_sink;
    int report_count = (int)(beacon_count/((double)report_period/(double)beacon_period));//each report packet will be generated by factor of beacon_count/(report_period/beacon_period) , only whole part of the factor will be considered
    
//setting our beacon and report cound onto the payload
