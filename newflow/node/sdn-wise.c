...
#define RF_SEND_K173713_EVENT  61
...
PROCESS(K173713_timer_proc, "K173713 Timer Process");
  ...
  while (1)
  {
    ...
    case RF_B_RECEIVE_EVENT:
      ...
      if (!conf.is_active)
      {
        ...
        process_post(&K173713_timer_proc, ACTIVATE_EVENT, (process_data_t)NULL);
    ...
    case RF_SEND_K173713_EVENT:
        rf_unicast_send(create_K173713(beacon_count,_REPORT_PERIOD,_BEACON_PERIOD));
        beacon_count = 0;
        #if !SINK
                if (conf.reset_period == 0){
                  conf.distance_from_sink = _MAX_DISTANCE;
...
PROCESS_THREAD(K173713_timer_proc, ev, data) {
    static struct etimer et;
...
#endif
      etimer_set(&et, conf.K173713_period * CLOCK_SECOND);
      PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&et));
      process_post(&main_proc, RF_SEND_K173713_EVENT, (process_data_t)NULL);
    }
    PROCESS_END();
  }
