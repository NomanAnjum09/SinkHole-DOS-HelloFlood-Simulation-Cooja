...
/*----------------------------------------------------------------------------*/
  ...
  void handle_MMD(packet_t *p){
    print_packet(p);
    printf("\n (Handling MMD) Node Number Before Broadcast: %d.%d\n",conf.my_address.u8[0],conf.my_address.u8[1]);

    int y = 0 ;
    for (y = 0 ; y <= (int)p->payload[0]; y++) {
    ...
    // after broadcast malicous node
    // is removed from neighbor table

    printf("\n (Handling MMD) Removing Malicous Node From Neighbour Table After Broadcast");

    y=0;
    for(y=1;y<=(int)p->payload[0];y++){
      remove_neighbour_node(&blacklistedMotes[y-1]);
    }
